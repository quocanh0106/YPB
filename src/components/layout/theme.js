...
class TabsComponent extends HTMLElement {
  ...
  handleTabClick(clickedTab) {
    if (!clickedTab.classList.contains('active')) {
      ...
      if(this.querySelector('select-toggle')) {
        this.accordion = this.querySelector('select-toggle')
        this.summary = this.accordion.querySelector('.summary');
        this.details = this.accordion.querySelector('.details');

        this.summary.querySelector('span').textContent = clickedTab.textContent;
        this.summary.querySelector('span').dataset.value = clickedTab.dataset.value;
        this.details.style.maxHeight = 0;
  ...
  hideTab(tabId) {
    if (this.classList.contains('normal-tab')) {
      ...
      this.querySelector(`.tab-detail[data-id='${tabId}']`).classList.add('lg:hidden');
    }
